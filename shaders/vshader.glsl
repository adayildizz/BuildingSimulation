...
uniform mat4 gVP;           // Combined View * Projection matrix (camera)
...
void main()
{
    ...
    vec4 worldPos_vec4 = gModelMatrix * vPosition; // vPosition is already vec4 from file
    outWorldPos = worldPos_vec4.xyz;

    // Transform vertex position to clip space (camera)
    gl_Position = gVP * worldPos_vec4;
    
    // Transform normal to world space
    outNormal_world = normalize(mat3(gModelMatrix) * vNormal); // Assuming gModelMatrix has no non-uniform scaling
    
    baseColor = vColor; // Pass original vertex color (if used)
    outTexCoord = vTexCoord;

    // NEW: Transform fragment position to light space for shadow mapping
